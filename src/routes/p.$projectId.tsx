...
function PublicChangelog() {
  ...
  const { data: project } = useSuspenseQuery(
    convexQuery(api.projects.getProjectPublic, {
      projectId: projectId as Id<'projects'>,
    }),
  ) as { data: Doc<'projects'> | null };
  const { data: commits } = useSuspenseQuery(
    convexQuery(api.projects.listCommitsPublic, {
      projectId: projectId as Id<'projects'>,
    }),
  ) as { data: Doc<'commits'>[] };
  ...
  if (!project) {
    return (
      <div className="p-8">
        <p>Project not found or not public.</p>
    ...
    <div className="mx-auto max-w-3xl space-y-8 p-8">
      ...
      <header>
        ...
        <p className="text-slate-600 dark:text-slate-400">
          {project.owner}/{project.repo} ({project.branch}) · Public changelog
        </p>
      </header>
